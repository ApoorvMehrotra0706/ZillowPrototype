...
class ApplyRenter extends React.Component {
  ...
  applyRenter = () => {
    ...
    if (error === 0) {
      ...
      var CreditScore = Number(this.state.CreditScore, 10);
      let data = {
        ...
        CreditScore,
        EmpInformation: this.state.EmpInformation,
      ...
      axios
        .post(configPath.api_host + `/renter/fileApplication`, data)
        .then((response) => {
        ...
        <Modal.Body>
          ...
          <Form.Group controlId="formBasicPassword">
            ...
            <Form.Control
              ...
              value={this.state.EmpInformation}
              onChange={(e) => {
                this.setState({ EmpInformation: e.target.value });
              }}
            />
          </Form.Group>
        </Modal.Body>
...
export default connect(mapStateToProps)(ApplyRenter);
